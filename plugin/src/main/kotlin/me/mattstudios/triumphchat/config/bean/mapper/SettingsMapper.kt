...
class SettingsMapper : PropertyMapper {
    ...
    fun mapFormatDisplay(path: String, data: Map<String, Any>): FormatDisplay {
        ...
        if (MESSAGE_PLACEHOLDER in text) return FormattedDisplay(text, hover, click, MESSAGE_PLACEHOLDER)
     ...
     */
    @TargetObject(FormattedDisplay::class)
    fun mapMessageDisplay(path: String, data: Map<String, Any>): FormattedDisplay {
        val rawText = data["text"]
        val text = if (rawText is String) rawText else ""
        return FormattedDisplay(text,  createHover(data["hover"]), createClick(data["click"]))
    }
